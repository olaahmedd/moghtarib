  ...
  Widget build(BuildContext context) {
    ...
    return BlocListener<SanaieeCubit, SanaieeState>(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          title: const Text('Sanaiee Details', style: TextStyle(color: Colors.white, fontSize: 18)),
          backgroundColor: Colors.transparent, // 🎯 شفاف عشان التدرج يبان
          elevation: 0,
          leading: IconButton(
            icon: const Icon(Icons.arrow_back, color: Colors.white),
            onPressed: () => Navigator.pop(context), 
          ),
          // 🎯 الألوان المدرجة للـ AppBar الجديد
          flexibleSpace: Container(
            decoration: const BoxDecoration(
              gradient: LinearGradient(
        ...
        body: SingleChildScrollView(
          ...
          child: Column(
            children: [
              ...
              // 🎯 تعديل شارة القسم (Badge): لو ليه قسم بيعرضه، لو كاتب مهنته يدويًا بتظهر هنا علطول
              if (user.departmentName != null && user.departmentName!.isNotEmpty) ...[
                const SizedBox(height: 10),
                _buildDepartmentBadge(user.departmentName!),
              ] else if (user.websiteURL != null && user.websiteURL != "string" && user.websiteURL!.isNotEmpty) ...[
                const SizedBox(height: 10),
                _buildDepartmentBadge(user.websiteURL!), // عرض المهنة المكتوبة يدويًا
              ],
                ...
                child: Column(
                  children: [
                    ...
                    _buildDetailRow(
                      Icons.language, 
                      'Website URL', 
                      (user.websiteURL == "string") ? null : user.websiteURL, // إخفاء النص الافتراضي لو مش حقيقي
                    ),
                    _buildDetailRow(Icons.admin_panel_settings_outlined, 'Account Type', user.type),
                    _buildDetailRow(
                      Icons.work_outline, 
