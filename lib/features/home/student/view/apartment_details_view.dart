...
// الموديل بتاعكِ
import 'package:moghtarib/features/home/semsar/view/add_apartment_tap_view.dart'; // شاشة الإضافة
...
class _ApartmentDetailsViewState extends State<ApartmentDetailsView> {
  ...
  IconButton(
    ...
    onPressed: () {
      // 🎯 بنفتح شاشة التعديل مباشرة وبنمرر بيانات الشقة الحالية
Navigator.push(
  context,
  MaterialPageRoute(
    builder: (context) => BlocProvider(
      // 🎯 بنصنع الكيوبيت عشان نمنع الشاشة الحمراء
      create: (context) => ApartmentCubit(ApartmentRepo()), 
      child: AddApartmentTabView( 
        // 🎯 بنمرر الشقة الحالية للشاشة عشان تفتح في وضع التعديل (Edit Mode)
        apartmentToEdit: widget.apartment, 
      ),
    ),
  ),
);
  ...
  Widget _buildDetailInfoTile({required String title, required String value, Color? valueColor}) {
    return Column(
      ...
      children: [
        ...
        const SizedBox(height: 4),
        Text(value, style: TextStyle(color: valueColor ?? Colors.white, fontSize: 22, fontWeight: FontWeight.bold)),
      ],
    );
  }
