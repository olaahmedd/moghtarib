  ...
  Widget build(BuildContext context) {
    ...
    try {
      final String? token = CacheHelper.getValue(CacheKeys.accessToken)?.toString() ?? 
                            CacheHelper.getValue('token')?.toString();
      
      if (token != null && token.isNotEmpty) {
        Map<String, dynamic> decodedToken = JwtDecoder.decode(token);
        currentUserRole = decodedToken['http://schemas.microsoft.com/ws/2008/06/identity/claims/role']?.toString() ?? 
    ...
    currentUserRole = currentUserRole.trim().toLowerCase(); 
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          ...
          if (currentUserRole != 'student' && currentUserRole.isNotEmpty) 
            IconButton(
